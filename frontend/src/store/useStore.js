...
export const useStore = create((set, get) => {
    ...
    const applyHobbyFilter = (nodes, edges) => {
    ...
    const updateGraphFromData = (nodes, edges) => {
    ...
    return {
        ...
        linkUsers: async (userId, friendId) => {
            try {
                ...
                const exists = get().edges.some((e) => (e.source === userId && e.target === friendId) || (e.source === friendId && e.target === userId));
                if (!exists) {
                    set({
                        edges: [
                            ...get().edges,
                            { id: `${userId}-${friendId}`, source: userId, target: friendId, type: 'smoothstep', animated: true },
                        ],
                    });
                    // bump friend counts
                    set({
                        nodes: get().nodes.map((n) => n.id === userId || n.id === friendId
                            ? { ...n, data: { ...n.data, friendCount: n.data.friendCount + 1 } }
                            : n),
                    });
                }
            }
        ...
        unlinkUsers: async (userId, friendId) => {
            try {
                ...
                set({
                    edges: get().edges.filter((e) => !((e.source === userId && e.target === friendId) || (e.source === friendId && e.target === userId))),
                });
                // reduce friend counts, floor at 0
                set({
                    nodes: get().nodes.map((n) => n.id === userId || n.id === friendId
                        ? { ...n, data: { ...n.data, friendCount: Math.max(0, n.data.friendCount - 1) } }
                        : n),
                });
            }
            catch (error) {
